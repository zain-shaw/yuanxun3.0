...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.fan.set_state",
        "Set the state of the fan. Can be 'on' or 'off'.",
        PropertyList({
            Property("state", kPropertyTypeString)
        }),
        [&board](const PropertyList& properties) -> ReturnValue {
            std::string state = properties["state"].value<std::string>();
            bool fan_state = (state == "on");
            board.SetFanState(fan_state);
            return true;
        });

    // 添加RGB灯控制工具
    AddTool("self.rgb_led.set_color",
        "Set the color of the RGB LED. Values range from 0 to 255 for each channel.",
        PropertyList({
    ...
    // 添加系统自我介绍工具
    AddTool("self.get_introduction",
        "Get the system's self introduction information. Use this tool when the user asks about the system's identity or to introduce itself.",
        PropertyList(),
        [](const PropertyList& properties) -> ReturnValue {
            return "{\"introduction\":\"我是元巡，一个AI 全域智核巡检智能机器人，时刻守护你的安全。\"}";
        });
...
void McpServer::ParseMessage(const cJSON* json) {
    ...
    if (method_str == "initialize") {
        ...
        std::string message = "{\"protocolVersion\":\"2024-11-05\",\"capabilities\":{\"tools\":{}},\"serverInfo\":{\"name\":\"元巡\",\"version\":\"";
